    ...
    : ThreadedLoop("quantum_hand", 0.)
{
    ...
    _menu->add_item("s", "Stop", [this](std::string) { makeContraction(STOP); });
    _menu->add_item("c", "Contraction (1-2)", [this](std::string args) { if(args.length() == 0) args = "1"; makeContraction(SHORT_CONTRACTION, std::stoi(args)); });
...
void QuantumHand::loop(double, clock::time_point)
{
    if (_cfg.opening_before) {
        ...
        std::this_thread::sleep_for(std::chrono::microseconds(_cfg.time_between_contractions_us*3));
    }

    for (; _cfg.n_contractions > 0; --_cfg.n_contractions) {
        if (_cfg.emg1_activated && !_cfg.emg2_activated) {
            _dac->analogWrite(2,_cfg.intensity*250);
            _dac->analogWrite(3,0);
        } else if (!_cfg.emg1_activated && _cfg.emg2_activated){
            _dac->analogWrite(2,0);
            _dac->analogWrite(3,_cfg.intensity*250);
        } else if (_cfg.emg1_activated && _cfg.emg2_activated) {
            _dac->analogWrite(0, 0, _cfg.intensity*250, _cfg.intensity*250);
        } else {
            _dac->analogWrite(2,0);
            _dac->analogWrite(3,0);
        }

        if (_cfg.time_limited) {
...
void QuantumHand::makeContraction(CONTRACTION_TYPE contraction_type, int emg, uint16_t intensity, bool opening)
{
    ...
    _cfg.time_contraction_us = 150000;
    ...
    _cfg.time_opening_before_us = 1000000;
    _cfg.time_limited = true;

    switch (contraction_type) {
    case SHORT_CONTRACTION:
        _cfg.time_contraction_us = 100000;
        _cfg.time_limited = false;
        break;
