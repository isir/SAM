...
SAManager::~SAManager()
{
    ...
    if (_robot->joints.epos1)
        _robot->joints.epos1->CloseDevice();
    if (_robot->joints.epos2)
        _robot->joints.epos2->CloseDevice();   
...
void SAManager::fill_menus()
{
    ...
    _main_menu->add_submenu_from_user(_robot->joints.epos1);
    _main_menu->add_submenu_from_user(_robot->joints.epos2);
