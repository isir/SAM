...
optitrack_data_t OptiListener::unpack(char *pData) {
    ...
    if(tmpData.messageID == 7)      // FRAME OF MOCAP DATA packet
    {
        ...
        memcpy(&tmpData.frameNumber, ptr, 4); ptr += 4;
        ...
        memcpy(&tmpData.nMarkerSets, ptr, 4); ptr += 4;
        ...
        for (int i=0; i < tmpData.nMarkerSets; i++)
        {
            ...
            memcpy(&tmpMarkerSet.nMarkers, ptr, 4); ptr += 4;
            //qDebug("Marker Count : %d", tmpMarkerSet.nMarkers);

            for(int j=0; j < tmpMarkerSet.nMarkers; j++)
            {
                optitrack_marker_t tmpMarker;
                tmpMarker.posx = 0; memcpy(&tmpMarker.posx, ptr, 4); ptr += 4;
                tmpMarker.posy = 0; memcpy(&tmpMarker.posy, ptr, 4); ptr += 4;
                tmpMarker.posz = 0; memcpy(&tmpMarker.posz, ptr, 4); ptr += 4;
        ...
        tmpData.nOtherMarkers = 0; memcpy(&tmpData.nOtherMarkers, ptr, 4); ptr += 4;
        // OtherMarker list is Deprecated
        //printf("Unidentified Marker Count : %d\n", nOtherMarkers);
        for(int j=0; j < tmpData.nOtherMarkers; j++)
        {
            optitrack_marker_t tmpMarker;
            tmpMarker.posx = 0.0f; memcpy(&tmpMarker.posx, ptr, 4); ptr += 4;
            tmpMarker.posy = 0.0f; memcpy(&tmpMarker.posy, ptr, 4); ptr += 4;
            tmpMarker.posz = 0.0f; memcpy(&tmpMarker.posz, ptr, 4); ptr += 4;
        ...
        memcpy(&tmpData.nRigidBodies, ptr, 4); ptr += 4;
        ...
        for (int j=0; j < tmpData.nRigidBodies; j++)
        {
            ...
            tmpRigidBody.ID = 0; memcpy(&tmpRigidBody.ID, ptr, 4); ptr += 4;
            tmpRigidBody.x = 0.0f; memcpy(&tmpRigidBody.x, ptr, 4); ptr += 4;
            tmpRigidBody.y = 0.0f; memcpy(&tmpRigidBody.y, ptr, 4); ptr += 4;
            tmpRigidBody.z = 0.0f; memcpy(&tmpRigidBody.z, ptr, 4); ptr += 4;
            tmpRigidBody.qx = 0; memcpy(&tmpRigidBody.qx, ptr, 4); ptr += 4;
            tmpRigidBody.qy = 0; memcpy(&tmpRigidBody.qy, ptr, 4); ptr += 4;
            tmpRigidBody.qz = 0; memcpy(&tmpRigidBody.qz, ptr, 4); ptr += 4;
            tmpRigidBody.qw = 0; memcpy(&tmpRigidBody.qw, ptr, 4); ptr += 4;
            ...
            {
                // Mean marker error
                tmpRigidBody.fError = 0.0f; memcpy(&tmpRigidBody.fError, ptr, 4); ptr += 4;
            ...
            {
                // params
                tmpRigidBody.params = 0; memcpy(&tmpRigidBody.params, ptr, 2); ptr += 2;
